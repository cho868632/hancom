...
document.querySelector("#bStr").addEventListener("click", () => show("10")); // String
...
document.querySelector("#bUndef").addEventListener("click", () => show()); // undefined (값 미할당)
document.querySelector("#bNull").addEventListener("click", () => show(null)); // null → typeof는 "object" (유명한 버그)
document
  .querySelector("#bArr")
  .addEventListener("click", () => show([1, "Bob", 10])); // Array → object
document
  .querySelector("#bObj")
  .addEventListener("click", () => show({ name: "Bob" })); // Object
